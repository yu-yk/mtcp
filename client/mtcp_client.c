...
static void *send_thread(void *server_arg){
	...
	// send SYN to server
	sendto(server_arg.socket_fd, SYN.buffer, strlen(SYN.buffer)+1, 0, server_arg.server_addr, sizeof(server_arg.server_addr));
	...
	// send ACK to server
	sendto(server_arg.socket_fd, ACK.buffer, strlen(ACK.buffer)+1, 0, server_arg.server_addr, sizeof(server_arg.server_addr));
...
static void *receive_thread(void *server_arg){
	...
	if(recvfrom(server_arg.socket_fd, buf, sizeof(buf), 0, server_arg.server_addr, sizeof(server_arg.server_addr)) < 0)
	{
